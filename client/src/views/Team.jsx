    ...
    <main className="flex flex-col justify-center items-center">
      ...
      <div className="mt-20 grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-12">
        {loaded
          ? teamMember
          .filter((member) => {
            if (member.extra_info.position && member.extra_info.secureUrl) {
              return true;
            }
            else {
              return false;
          ...
          .map((member) => (
              <TeamCard
                ...
                id={member.userId}
                position={member.extra_info.position}
                github={member.extra_info.github}
                linkedin={member.extra_info.linkedin}
                image={member.extra_info.secureUrl}
                loaded={loaded}
              />
            ))
          : [...Array(9)].map(
